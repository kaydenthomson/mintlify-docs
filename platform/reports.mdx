...
Reports allow you to create, publish, and share customizable, presentation-ready pages using the Page Editor. Unlike traditional financial reports, these are professional, branded pages built from content blocks and data, shared as secure, view-only links with delivery analytics.

**Key Benefits**:

- Create polished updates and summaries without exporting PDFs or combining documents.
- Reuse Page Editor blocks for consistent formatting.
...
To create a report, you must have access to an organization. Follow these steps:

1. Go to **Organization > Reports** (`/[orgId]/reports`).
2. Click **New Report**.
   - A draft report opens in the Page Editor.
   - Optionally, select or create a report group to organize the report.
3. Enter a **Name** and optional **Description** in the breadcrumb on the report page.
   - The name becomes the page title and can be edited inline.
...
To create a report, click **New Report** to start a draft in the Page Editor. To move a report to a group, use the table’s context menu.

### Edit a Report

<DevOnly>[Insert screenshot of report builder UI]</DevOnly>

The Page Editor powers report creation with rich text and structured blocks. Key features include:

- **Auto-Save**: Changes save automatically with a short delay. No manual save is needed.
- **Read-Only After Publish**: Once sent, the report becomes read-only to preserve content for recipients.
- **Cleanup**: The app removes unused files referenced in the page to optimize storage.

**Supported Blocks and Formatting**:

- Headings and paragraphs
- Lists (bulleted, numbered, checklists)
- Links with a floating toolbar
- Callouts and toggles
- Code blocks
- Tables and columns
- Mentions, suggestions, comments, and basic formatting (bold, italic, underline, code)
- Table of contents (if enabled in templates)
- Inline color and formatting tools

**Note**: Features may vary based on configuration or updates.

### Rename a Report

<DevOnly>[Insert screenshot of inline title editing]</DevOnly>

1. Click the report title in the breadcrumb.
2. Enter a new name and press **Enter** to save.
...
1. Open the report and click **Publish & Send** (for drafts) or **Send** (for sent reports).
2. Choose an option:
   - **Send a Test**: Sends the report to your verified email for review.
   - **Send to Recipients**: Select contacts or lists using the Recipients selector. The system merges and deduplicates recipients.
3. Click **Send Report** to email a secure, view-only link.
   - On the first send, the report status changes to "sent," and the "Sent at" timestamp is recorded.
...
<DevOnly>[Insert screenshot of data blocks in Page Editor]</DevOnly>

Data blocks enhance reports by embedding dynamic financial and operational metrics directly in the Page Editor. These blocks pull live data from connected sources, such as accounting platforms (e.g., QuickBooks), to create insightful, professional visuals and summaries.
...
- Add the Benchmarks block in the Page Editor, choose the period, scope, and KPIs, then publish.
...
<Update label="Data blocks roadmap" description="In development">
  Additional data blocks will soon extend the Page Editor so you can spotlight live metrics in different formats.
...
- **Customizable Layouts**: Use the Page Editor to design templates with reusable blocks and formatting.
- **Quick Generation**: Create a new report from a template with one click, pre-filled with data and recipients.

**How to Use**:

1. Go to **Organization > Reports > Templates** (`/[orgId]/reports/templates`).
...
<AccordionGroup>
  <Accordion title="Draft reports">
    - Fully editable. - Supports test sends to your email. - Converts to "sent" on the first successful send.
  </Accordion>
  <Accordion title="Sent reports">
    - Read-only to preserve content. - Allows sending to additional recipients. - Displays analytics.
  </Accordion>
</AccordionGroup>
...
- Use the <Tooltip tip="Search and select contacts or lists stored for the organization">Recipients Multiselect</Tooltip> to choose contacts or lists at `/[orgId]/contacts`.
...
<AccordionGroup>
  <Accordion title="Can't create a report?">
    - Verify you have organization access and are on the Reports page (`/[orgId]/reports`). - Confirm the page
    loads without permission errors before creating drafts.
  </Accordion>
  <Accordion title="Send errors or failed recipients">
    - Check that email addresses are valid and verified. - Reduce recipient count if approaching the
    100-recipient limit.
  </Accordion>
  <Accordion title="Recipient can't access the report">
    - Ensure their email matches the sent recipient list and is verified. - Ask recipients to sign in with the
    same email and refresh the secure link.
  </Accordion>
  <Accordion title="Analytics not updating">
    - Allow time for delivery and engagement signals to update, then refresh the page. - Remind viewers that
    "Viewed" requires a signed-in session with a matching verified email.
  </Accordion>
</AccordionGroup>
...
- **Report**: A shareable, view-only page created in the Page Editor.
- **Page Editor**: The rich text editor for building report content with blocks.
...
[Insert short video clip of creating, editing, and sending a report]
